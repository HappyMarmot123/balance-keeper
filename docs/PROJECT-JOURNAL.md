...
- 상태: `ACCEPTED` — D-065의 incidents+disasters 수직 슬라이스를 오토모드로 검증·수락했고 dangerous-car는 안전 계약 해제 전 `FEATURE_OFF`다.
- 목적: 현재 교통 돌발과 재난 위치를 T30에서 활성화할 수 있는 엄격한 `road-event` 데이터 수직 슬라이스로 준비한다.
- 포함:
  - 전국 고정 bbox의 `/api/road-events/incidents`, 최근 7일 전국 고정 범위의 `/api/road-events/disasters`
  - 공통 event identity·category·lifecycle·`provider-unspecified` severity와 point/line/area 좌표 계약
  - source별 strict provider, 독립 cache·budget·breaker, 기본 비활성 Query
  - T26 disaster validator를 숫자 geometry type과 bare coordinate sequence로 교정
- 제외: Widget·지도·사용자 bbox, 종료 이력 공개, raw provider ID, 위험물 차량/사고 정밀 위치 route, 임의 truncation, 신규 dependency.
- 완료 조건·검증:
  - 정상: array/object/공식 empty 변형, KST 날짜와 12·14자리 종료, event point와 disaster type `1|2|3` geometry, deterministic dedup·정렬을 검증한다.
  - 실패: non-200·redirect·MIME·UTF-8·JSON·provider code·count·scope·bbox·geometry·credential reflection·payload 상한을 fail-closed한다.
  - 경계: blank message/end, calendar·KST 자정, expired 제거, unknown enum, duplicate conflict, lowercase/uppercase disaster field와 공식에 없는 WKT를 검증한다.
  - 회귀: focused tests와 `npm run validate`, FSD·gateway·security 독립 리뷰를 통과하고 기존 route/cache/build를 보존한다.
- 구현·판단:
  - provider-neutral `road-event` Entity, incidents 2분·disasters 5분 기본 비활성 Query, 독립 cache·budget·breaker를 가진 두 gateway route를 production runtime에 등록했다.
  - T26 disaster geometry를 숫자/문자 `1|2|3`과 bare `X Y,...` 계약으로 교정하고 문서에 없는 WKT·충돌 casing·유효하지 않은 점 개수를 fail-closed했다.
  - 공식 종료 status(D03 해제군, D06 `3`, D07 `2|3`)는 endDate가 비어도 제외한다. 고유 ID가 없는 incident는 위치·시작·유형·link 기반 occurrence key를 쓰고 가변 message·end·lane revision에 ID를 유지하며 동시 충돌 revision은 거부한다.
  - disaster 공식 nullable/생략 필드와 12·14자리 endDate를 허용하되 스키마·cardinality·scope·크기·credential reflection은 엄격히 거부한다.
- 검증:
  - 마지막 리뷰 finding 2건에 대한 RED `8 FAIL` 후 provider focused `27/27 PASS`; Entity·provider·route·runtime·T26 회귀 `112 PASS`, live gate 2개 channel `PASS`, gated live 2건 `SKIP`.
  - `npm run validate`: 185 files PASS·12 gated SKIP, 1,749 tests PASS·14 gated SKIP, strict typecheck·client/server build PASS. 기존 lazy HLS 500 KiB warning은 변경 없다.
  - 서버가 Entity Query barrel을 경유하던 FSD 경계 finding을 contract direct import로 해소했다. 독립 correctness·security·gateway·FSD 재리뷰는 재현 가능한 잔여 finding 0건으로 PASS했다.
...
- 게시: feature commit `d284761`, PR #28 quality-gate SUCCESS 후 merge commit `9e67988`로 `development`에 반영했다.
...
- 포함:
  - `road-traffic` Entity에 bounds, link/node metadata, unit-neutral speed, seconds travel time, provider 14자리 source timestamp 계약 추가
  - 기본 비활성 bbox Query, strict ITS provider, coarse gateway `/api/road-traffic/current`, production runtime 등록
  - bbox 각 축 최대 0.1°, 소수 4자리 canonicalization, raw/public 2 MiB와 10,000 segment fail-closed 상한
  - synthetic offline RED→GREEN과 값 미출력 credential-gated small-bbox live smoke
- 제외: 표준 노드링크 geometry·지도 overlay·혼잡등급·속도 단위 추론, 전국/광역 bbox, detector, Vercel 배포.
- 완료 조건:
  - 정상: canonical bbox가 upstream 동일 bbox로 전달되고 strict snapshot과 Query key/cache identity에 일치한다.
  - 실패: 누락·중복·비정규 bbox, span 초과, redirect/MIME/UTF-8/schema/count/size/credential 오류가 public detail이나 secret 없이 fail-closed 한다.
  - 경계: empty·single/object·array rows, nullable provider labels, duplicate/conflicting link identity, timestamp/calendar, 숫자 상한을 검증한다.
  - 회귀: forecast API와 T30 지도 registry는 바뀌지 않고 focused·전체 `npm run validate`, live smoke, 독립 review가 PASS한다.
- 근거:
  - ITS 공식 페이지는 `type=all`에서 bbox 4개를 허용하고 output을 link/node metadata, speed, seconds travel time, `YYYYMMDDHH24MISS`로 정의하며 geometry는 제공하지 않는다.
  - 값 미출력 preflight: 서울 0.1°×0.1°는 4,417 rows·약 866 KiB, 0.05°×0.05°는 884 rows·약 173 KiB; 더 큰 0.5°×0.35°는 40,988 rows·약 8.06 MiB였다. 따라서 0.1°를 product-owned hard limit으로 동결하고 초과 시 확대를 요구한다.
- 예상 변경: `src/entities/road-traffic`, `src/server/providers/its`, `src/server/routes/road-traffic`, `src/server/runtime/productionRuntime.ts`, synthetic fixture와 Entity/provider/route/runtime/live/architecture tests, 이 Task 저널. UI·의존성·환경변수는 변경하지 않는다.
- RED→GREEN:
  - current model/query/presence 6건이 missing contract로 RED였고, bbox canonicalization·strict snapshot·기본 disabled 5분 Query로 GREEN했다.
  - provider 18건과 route/runtime 18건이 missing slice·404로 RED였고, strict `trafficInfo` parser, bbox cache identity, 5분 profile·500/day budget·MISS/HIT/304/STALE로 GREEN했다.
  - final response URL이 같은 path에서 query만 바뀌어도 통과하는 경계를 별도 RED로 재현해 요청 URL 전체 일치를 강제했다.
  - 독립 서버 리뷰가 HTTP 206 부분 응답과 direct/JSON-escaped credential reflection을 실제 성공으로 재현했다. exact 200만 허용하고 UTF-8 raw body와 parse된 모든 key/string value에서 key 반사를 차단해 회귀 2건을 GREEN했다.
- 검증 증거:
  - current·forecast·map regression focused 12 files 90 tests PASS·live gate 1 skipped; 서버 최종 보안 재리뷰 focused 5 files 43 tests PASS.
  - `RUN_ITS_ROAD_TRAFFIC_CURRENT_LIVE_SMOKE=1` + 추적 제외 local credential로 서울 0.05°×0.05° strict snapshot 1/1 PASS; key와 provider 값은 출력·저장하지 않았다.
