...
const serviceKey = process.env.KOREA_EARTHQUAKE_KEY?.trim();
  ...
  liveIt(
    ...
    async () => {
      if (serviceKey === undefined || serviceKey.length === 0) {
        throw new TypeError('KOREA_EARTHQUAKE_KEY is required for the live smoke');
      }
      ...
      const runtime = createProductionGatewayRuntime({
        ...
        environment: { KOREA_EARTHQUAKE_KEY: serviceKey },
